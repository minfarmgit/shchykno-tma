...
- `POST /api/tilda-webhook` принимает `application/x-www-form-urlencoded` submit из Tilda, ожидает webhook token в headers, сохраняет raw payload и фиксирует покупку по `tranid`.
...
- `POST /api/telegram-bot-webhook` принимает webhook от Telegram-бота и сохраняет номер телефона, когда пользователь делится контактом из Mini App.
...
- Tilda должна слать формы на `https://your-domain.com/api/tilda-webhook`.
- API key нужно передавать в headers, например: `X-Tilda-Webhook-Token: <TILDA_WEBHOOK_SECRET>`.
...
- Настройте webhook бота на `https://your-domain.com/api/telegram-bot-webhook` и передавайте `X-Telegram-Bot-Api-Secret-Token`, равный `TELEGRAM_BOT_WEBHOOK_SECRET`.
